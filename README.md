...
Version-specific helpers to convert raw `SessionProfile` numbers into the correct OCPP `SetChargingProfile` payload. Import from the `/builders` subpath.

| Helper | OCPP Version | Output field name | `chargingSchedule` shape |
| ---------------------- | ------------ | ----------------- | ------------------------ |
| `buildOcpp16Profile()` | 1.6 | `csChargingProfiles` | single object |
| `buildOcpp201Profile()` | 2.0.1 / 2.1 | `chargingProfile` | **array** |

**Why the difference?** OCPP 2.0.1 made `chargingSchedule` an array to support multiple tariff tiers, and renamed `chargingProfileId` → `id` and `transactionId` from `integer` to `string`.

Both builder functions accept optional second argument for `stackLevel`, `purpose`, `rateUnit` (`"W"` or `"A"`), and `numberPhases`.
